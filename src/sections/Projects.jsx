...
const projects = useMemo(() => [
      {
        title: "Dentwise",
        link: "https://dentwise-c5qb.onrender.com",
      ...
      {
        title: "Thumblify",
        link: "https://ai-thumbnail-generator-sooty.vercel.app",
        codeLink: "https://github.com/TrishitaGayen/ai-thumbnail-generator",
        bgColor: "#DF7FE3",
        image: isMobile ? photo2 : img2, 
      },
      {
        title: "CareBotics",
        link: "https://customer-support-two-lemon.vercel.app/",
        codeLink: "https://github.com/TrishitaGayen/customer-support",
        bgColor: "#8AEBE5",
        image: isMobile ? photo4 : img4, 
      },
      {
        title: "InvoiceAI",
        link: "https://ai-invoice-generator-ui.onrender.com",
        codeLink: "https://github.com/TrishitaGayen/ai-invoice-generator",
        bgColor: "#8AA4EB",
      ...
      <div className="sticky top-0 h-screen flex flex-col items-center justify-center">
          ...
          {projects.map((project, idx) => (
            <div key={project.title} className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transition-all duration-500 ${activeIndex === idx ? "opacity-100 z-20" : "opacity-0 z-0 sm:z-10"}`}
              ...
              <AnimatePresence mode="wait">
                {activeIndex === idx && (
                  <motion.h3 key={project.title} className={`block text-center text-[clamp(2rem,6vw,5rem)] text-white/95 sm:absolute sm:-top-20 sm:left-[35%] lg:left-[-5%] sm:mb-0 italic font-semibold
                    ${isMobile ? "-mt-24" : ""}`} 
                    style={{
                      zIndex: 5,
                      textAlign: isMobile ? "center" : "left",
                    }} 
                    initial={{opacity:0 , y:-30}} animate={{opacity:1 , y:0}} exit={{opacity:0 , y:30}} transition={{duration:0.5 , ease: "easeOut"}}>
                    {project.title}
                  </motion.h3>
                )}
              </AnimatePresence>
        ...
        <div className={`absolute ${isMobile ? "bottom-20" : "bottom-10"} flex gap-4`}>
          ...
          <a href={activeProject?.codeLink} target="_blank" rel="noopener noreferrer" 
            className="inline-block px-6 py-3 font-semibold rounded-lg bg-teal-700 text-white hover:bg-teal-400 transition-all"
