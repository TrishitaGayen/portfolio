    ...
    <>
      ...
      ${visible ? "translate-y-0" : "-translate-y-full"}`}>
        ...
        <div className="flex items-center space-x-2">
          <img src={Logo} alt="LogoImage" className="w-10 h-10 brightness-125 contrast-125 drop-shadow-lg transition duration-300"/>
          <div className="text-2xl font-bold text-white sm:block">
            Trishita
          </div>
        </div>

        <div className="block lg:absolute lg:left-1/2 lg:transform lg:-translate-x-1/2">
          <button onClick={() => setMenuOpen(true)} className="text-white text-3xl focus:outline-none" aria-level="open menu">
            <FiMenu />
          </button>
        </div>
